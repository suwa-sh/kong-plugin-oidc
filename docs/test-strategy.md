...
計測ツール: luacov（`busted --coverage` で実行）

| スコープ | 閾値 | 根拠 |
|---------|------|------|
| ファイル別 | 95% 以上 | 到達不能コード（ローカル関数の未使用分岐等）を許容する余地 |
| 全体 | 95% 以上 | 初回計測 98.81%（2026-04-05 時点）をベースに、新規コード追加時の劣化を検知 |

**到達不能コードの扱い**: `set_consumer()` の consumer 非nil パス（4行）のように、公開APIから到達不能な防御的コードは閾値の例外とする。閾値未達の場合は、到達不能コードの一覧を PR に記載すること。

**CI での運用**: カバレッジが閾値を下回った場合は CI を失敗させる（CI/CD 導入時に `luacov` + 閾値チェックスクリプトを追加）。
